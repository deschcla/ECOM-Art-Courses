...
export class SousCategorieComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISousCategorie): number => this.sousCategorieService.getSousCategorieIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.sousCategories = this.refineData(dataFromBody);
  }

  protected refineData(data: ISousCategorie[]): ISousCategorie[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: ISousCategorie[] | null): ISousCategorie[] {
    return data ?? [];
  }
