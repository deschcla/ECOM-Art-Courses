...
export class AcheteurComponent implements OnInit {
  ...
  trackId = (_index: number, item: IAcheteur): number => this.acheteurService.getAcheteurIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.acheteurs = this.refineData(dataFromBody);
  }

  protected refineData(data: IAcheteur[]): IAcheteur[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IAcheteur[] | null): IAcheteur[] {
    return data ?? [];
  }
