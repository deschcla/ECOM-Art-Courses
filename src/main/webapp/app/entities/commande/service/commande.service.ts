...
type RestOf<T extends ICommande | NewCommande> = Omit<T, 'createdAt' | 'updateAt'> & {
...
export class CommandeService {
  ...
  protected convertDateFromClient<T extends ICommande | NewCommande | PartialUpdateCommande>(commande: T): RestOf<T> {
    return {
      ...commande,
      createdAt: commande.createdAt?.toJSON() ?? null,
      updateAt: commande.updateAt?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCommande: RestCommande): ICommande {
    return {
      ...restCommande,
      createdAt: restCommande.createdAt ? dayjs(restCommande.createdAt) : undefined,
      updateAt: restCommande.updateAt ? dayjs(restCommande.updateAt) : undefined,
    };
  }
